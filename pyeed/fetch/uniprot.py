...
class ProteinBuffer:
    ...
    def flush_to_neo4j(self, writer: Any) -> int:
        """
        Batch write to Neo4j using new add_protein method.

        Args:
            writer: PyeedNeo4jWriter instance
        ...
        for protein in self.proteins:
            try:
                ...
                if result["success"]:
                    ...
                    logger.error(
                        f"Failed to write protein {protein.accession_id}: {result.get('error', 'Unknown error')}"
                    )
            except Exception as e:
...
async def get_proteins_from_uniprot_batched(
    accessions: list[str], batch_size: int = 100, max_concurrent: int = 5
) -> list[Protein]:
    """
    High-performance batched fetcher for large numbers of proteins.
    ...
    logger.info(f"Fetching {len(accessions)} proteins in batches of {batch_size}")
    ...
    async def fetch_batch(batch_accessions: list[str]) -> list[Protein]:
        """Fetch a single batch of proteins."""
        async with semaphore:
            try:
                batch_proteins = await get_proteins_from_uniprot(batch_accessions)
                logger.info(
                    f"Batch complete: {len(batch_proteins)} proteins from {len(batch_accessions)} IDs"
                )
                return batch_proteins
            except Exception as e:
                logger.error(f"Batch failed for {len(batch_accessions)} IDs: {e}")
                return []
    ...
    logger.info(
        f"Batched fetch complete: {len(all_proteins)} proteins, {failed_batches} failed batches"
    )
    return all_proteins
